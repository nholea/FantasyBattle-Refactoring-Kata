...
public class PlayerTest {
    ...
    void calculatesDamageWhenPlayerIsEquippedWithFlashySwordOfDangerInHisRightHandAndOtherItems(int baseDamage, float damageModifier, int expectedDamage) {
    ...
    void calculatesDamageWhenPlayerIsEquippedWithExcaliburInHisRightHandAndOtherItems(int baseDamage, float damageModifier, int expectedDamage) {
        Inventory inventory = mock(Inventory.class);
        Stats stats = mock(Stats.class);
        SimpleEnemy target = mock(SimpleEnemy.class);
        when(inventory.getBaseDamage()).thenReturn(baseDamage);
